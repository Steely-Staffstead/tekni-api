...
public class GetOrderbookExcel
{
    private readonly string _connectionString = SqlEnv.GetConnectionString();

    [Function("GetOrderbookExcel")]
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Function, "get", Route = "orderbook/excel")] HttpRequestData req)
    {
        ...
        try
        {
            ...
            await using var cmd = new SqlCommand("api.usp_orderbook_get_excel", conn)
            {
