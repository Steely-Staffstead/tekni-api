...
public class GetOrderbookNormalized
{
    private readonly string _connectionString = SqlEnv.GetConnectionString();

    [Function("GetOrderbookNormalized")]
    public async Task<HttpResponseData> Run(
        [HttpTrigger(AuthorizationLevel.Function, "get", Route = "orderbook/normalized")] HttpRequestData req)
    {
        ...
        try
        {
            ...
            await using var cmd = new SqlCommand("api.usp_orderbook_get_normalized", conn)
            {
